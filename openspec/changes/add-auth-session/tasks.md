...
- [ ] 2.1 Завести обертку над `PasswordHasher<User>` из
      `Microsoft.Extensions.Identity.Core` - сборка есть в shared framework,
      `PackageReference` не нужен: хеширование и проверка, результат
...
- [ ] 4.4 Реализовать `POST /auth/signin`: проверка пароля, `SignInAsync`,
      ответ `200` с пустым телом и установкой cookie; неверные учетные данные -
