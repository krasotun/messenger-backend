...
- `GET /api/v2/health` продолжает отвечать `200` и
  `{ "status": "ok", "uptime": <целое> }` - единственное существующее
  поведение сохраняется без изменений, включая поле `uptime` с временем
  работы процесса в секундах.
