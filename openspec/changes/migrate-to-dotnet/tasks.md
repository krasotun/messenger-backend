...
- [x] 3.1 Подключить `Npgsql.EntityFrameworkCore.PostgreSQL` и
...
- [x] 3.2 Проверить цикл миграций от начала до конца на пустом `DbContext`:
      ...
      перед накатом.
- [x] 3.3 Убедиться, что старт приложения не зависит от доступности базы -
...
- [x] 6.1 Переписать блок `context` в `openspec/config.yaml`: стек, структура
      ...
      oxlint.
- [x] 6.2 Обновить `README.md`: установка SDK, поднятие базы, миграции,
...
- [x] 7.1 Прогнать полный набор проверок: `dotnet format
